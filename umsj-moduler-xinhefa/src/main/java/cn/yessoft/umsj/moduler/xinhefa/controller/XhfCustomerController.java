...
public class XhfCustomerController {
  ...
  @Resource private IXhfCustomerAuthService xhfCustomerAuthService;

  @Resource private IBaseAccountService baseAccountService;
            ...
            BeanUtils.toBean(
                ...
                customerVO -> {
                  if (customerVO.getMatchStrategy() != null) {
                    customerVO.setMatchStrategyStr(
                        XHFCusMatchStrategyEnum.valueOf(customerVO.getMatchStrategy()).getName());
                  }
                }),
            pageResult.getTotal()));
  }
  ...
  public ApiResult<List<CustomerAuthVO>> queryAuthList(
      @Valid @RequestBody CustomerQueryReqVO reqVO) {
    List<XhfCustomerAuthDO> r = xhfCustomerAuthService.queryAuthList(reqVO);
    return success(
        BeanUtils.toBean(
            r,
            CustomerAuthVO.class,
            customerVO -> {
              customerVO.setAccountName(
                  baseAccountService.get(customerVO.getAccountId()).getNickName());
            }));
  }
