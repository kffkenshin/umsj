...
public interface XhfItemMapper extends YesBaseMapper<XhfItemDO> {
    ...
    default PageResult<XhfItemDO> pagedQuery(ItemQueryVO reqVO) {
        return selectPage(reqVO, new LambdaQueryWrapperX<XhfItemDO>()
                .inIfPresent(XhfItemDO::getItemType1, reqVO.getItemType())
                .inIfPresent(XhfItemDO::getItemType2, reqVO.getItemType2())
                .and(BaseUtils.isNotEmpty(reqVO.getInfo()), w -> w.like(XhfItemDO::getItemNo, reqVO.getInfo())
                        .or().like(XhfItemDO::getItemName, reqVO.getInfo())
                        .or().like(XhfItemDO::getItemSpec, reqVO.getInfo()))
        );
    }

    default List<XhfItemDO> listQuery(ItemQueryVO reqVO) {
        return selectList(new LambdaQueryWrapperX<XhfItemDO>()
                .likeIfPresent(XhfItemDO::getItemNo, reqVO.getInfo())
                .or().likeIfPresent(XhfItemDO::getItemName, reqVO.getInfo())
                .or().likeIfPresent(XhfItemDO::getItemSpec, reqVO.getInfo())
        );
    }
